...
### `memory.md`
- 更新「上次完成」與「下一步」
- 在「Session 交接紀錄（最近 5 筆）」新增一行：`| 日期 | 完成事項 | 遺留問題 |`
- **壓縮檢查**：交接紀錄 ≥ 6 筆時，舊紀錄移入「歷史摘要（已壓縮）」，只保留最近 5 筆詳細列
...
- 完成里程碑後忘記 commit：每完成一個可運作單元（骨架、驗收通過、功能串接）必須立即 commit