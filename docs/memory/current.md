...
- `PRODUCT.1` and `PRODUCT.2` are complete. `ai.legacy_agent` observation-window
  items stay separate and non-blocking; parked tracks (`AGENT.*`, `CLIP.1`) stay frozen.
...
- `PRODUCT.3` trusted-release hardening started with a full-verify gate before
  release packaging; updater/signing/diagnostics/migration work remains.
