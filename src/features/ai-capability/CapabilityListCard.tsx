...
export function CapabilityListCard({
  ...
  const footerLabel = useMemo(() => {
    if (status === "pending") return "Loading workflow suggestions";
    if (status === "error") return "Could not load suggestions";
    if (status === "cancelled") return "Cancelled";
    if (status === "idle") return "Ready";
    if (!selected) return "No recent workflows";
    if (selected.replay) return "Arrow keys to navigate - Enter to replay";
    return "History suggestion only - replay not available yet";
  }, [selected, status]);
      ...
      <div className="kn-scroll max-h-[320px] overflow-y-auto px-2 py-2">
        {status === "pending" ? (
          <div className="px-2 py-3 text-sm text-[color:var(--kn-text-muted)]">
            Looking at recent workflows...
          </div>
        ) : status === "error" && error ? (
          <div className="px-2 py-3 text-sm text-rose-200">{error}</div>
        ) : status === "cancelled" ? (
          <div className="px-2 py-3 text-sm text-[color:var(--kn-text-muted)]">Cancelled.</div>
        ) : items.length === 0 ? (
          <div className="px-2 py-3 text-sm text-[color:var(--kn-text-muted)]">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <div className="mt-1 text-[10px] text-[color:var(--kn-text-faint)]">
                        {item.replay ? "Replay" : "History only"}
                      </div>
                    </div>
                  </div>
