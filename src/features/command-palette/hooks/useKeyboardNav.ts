// Keyboard navigation handler for the palette input.
//
...
export interface UseKeyboardNavDeps {
  ...
  /** True when capability mode is showing a navigable suggestion list. */
  ...
  /** Invoke the active capability stream's submit (Enter-to-ask). */
...
export function useKeyboardNav(deps: UseKeyboardNavDeps) {
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    // `?` opens cheatsheet when input is empty, so it doesn't
    // collide with typing `?` as part of a query.
    if (e.key === "?" && deps.query === "" && !deps.secondaryMenuOpen && !deps.cmdResult) {
      e.preventDefault();
      deps.setCheatsheetOpen(true);
      return;
    }

    // Capability mode owns Enter: fire `submit()` on the active
    ...
    if (deps.mode === "search") {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (deps.isArgsPhase) {
          ...
          deps.setSelectedCmd((i) => Math.min(i + 1, deps.cmdSuggestions.length - 1));
        }
      } else if (e.key === "ArrowUp") {
        e.preventDefault();
        if (deps.isArgsPhase) {
