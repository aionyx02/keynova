...
export function AiPanel({ onClose }: PanelProps) {
  ...
  return (
    <div className="bg-gray-900/95 backdrop-blur-md rounded-b-xl shadow-2xl flex flex-col" style={{ maxHeight: 3000 }}>
      {/* Header */}
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-3" style={{ minHeight: 560, maxHeight: 560 }}>
        ...
        {mode === "agent" && agent.runs.map((run) => (
          <div key={run.id} className="rounded bg-gray-800/60 px-3 py-2 text-sm text-gray-200">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-semibold uppercase text-blue-300">{run.status}</span>
              <button
            ...
            {run.output && <pre className="mt-2 whitespace-pre-wrap text-xs text-gray-400">{run.output}</pre>}
          </div>
        ))}
        <div ref={bottomRef} />
      </div>
