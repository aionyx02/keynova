...
目前下一步是補齊 Phase 4 尚未完成的重型 runtime 項目：search chunk streaming、`lib.rs` app 模組拆分、Tantivy provider、Agent approval/audit/memory、WASM runtime/hot reload。Phase 2 搜尋後端選擇、`search.backend` debug 顯示、`/rebuild_search_index` 本機索引重建入口、`[search]` 設定與 backend selection tests 已補齊；Phase 4.5A 已補上 read-only `keynova.search` 與 SearXNG `web.search` tool 入口。
...
- [x] Phase 4.5A Agent read-only tools：`agent.tool` 可呼叫 `keynova.search` / `web.search`，本機搜尋支援 workspace、commands、settings schema、models、notes、history，`web.search` 支援 SearXNG 且拒絕 private architecture / secret query。
...
- [x] Provider 支援 cancellation token / generation check。
...
- [x] 新增 `web.search` tool：透過設定 provider 執行基本網路搜尋，第一階段只回 title/url/snippet，不抓完整頁面。
- [x] `web.search` provider 介面支援 SearXNG / Brave Search / Tavily 之一或多個；API key 設為 sensitive setting，不寫入 log。
- [x] `web.search` query rewrite 不得包含 `private_architecture` / `secret` 內容，只能使用使用者問題與 allowlisted context。
- [x] 新增 `keynova.search` tool：搜尋 Keynova 本地內容與 runtime cache，但回傳前套用 visibility filter。
- [x] `keynova.search` 第一階段來源：notes metadata/content、clipboard/history 摘要、workspace summary、command/action metadata、model catalog、非敏感設定 schema。
